WoW_Speedrun_Timer = LibStub("AceAddon-3.0"):NewAddon("WoW_Speedrun_Timer", "AceConsole-3.0", "AceEvent-3.0")
...
local options = {
    name = "WoW_Speedrun_Timer",
    handler = WoW_Speedrun_Timer,
    type = "group",
    args = {
        msg = {
            type = "input",
...
function WoW_Speedrun_Timer:OnInitialize()
    -- Called when the addon is loaded
    self.db = LibStub("AceDB-3.0"):New("WoW_Speedrun_TimerDB", defaults, true)
    LibStub("AceConfig-3.0"):RegisterOptionsTable("WoW_Speedrun_Timer", options)
    self.optionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("WoW_Speedrun_Timer", "WoW_Speedrun_Timer")
    self:RegisterChatCommand("wst", "ChatCommand")
	debugprofilestart()
	self:Print("Addon loaded successfully!")
end

function WoW_Speedrun_Timer:OnEnable()
...
function WoW_Speedrun_Timer:OnDisable()
    -- Called when the addon is disabled
end

function WoW_Speedrun_Timer:ZONE_CHANGED()
...
function WoW_Speedrun_Timer:GetMessage(info)
    return self.db.profile.message
end

function WoW_Speedrun_Timer:SetMessage(info, newValue)
    self.db.profile.message = newValue
end

function WoW_Speedrun_Timer:IsShowInChat(info)
    return self.db.profile.showInChat
end

function WoW_Speedrun_Timer:ToggleShowInChat(info, value)
    self.db.profile.showInChat = value
end

function WoW_Speedrun_Timer:IsShowOnScreen(info)
    return self.db.profile.showOnScreen
end

function WoW_Speedrun_Timer:ToggleShowOnScreen(info, value)
    self.db.profile.showOnScreen = value
end

function WoW_Speedrun_Timer:ChatCommand(input)
    if not input or input:trim() == "" then
        InterfaceOptionsFrame_OpenToCategory(self.optionsFrame)
    else
        LibStub("AceConfigCmd-3.0"):HandleCommand("wst", input)
    end
end
